...
class DataProcessor:
    ...
    def normalize_yes_no(value) -> Optional[str]:
        ...
        # CORRECTION BUG: Gérer les types non-scalaires (listes de dicts du repeat group)
        # pd.isna() sur une liste retourne un array de booléens → ambiguïté
        if isinstance(value, (list, dict, np.ndarray, pd.Series)):
            return None
        try:
            if pd.isna(value):
                return None
...
def process_milda_dataframe(data: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
    ...
    column_mapping = {
            ...
            # CORRECTION BUG: geo_location est le nom du champ GPS dans le nouveau formulaire
            'latitude': ['latitude', '_LES COORDONNEES GEOGRAPHIQUES_latitude', '_geolocation', 'geo_location'],
            ...
            # CORRECTION BUG: 'scan_milda' est un begin_repeat → liste de dicts en KoBo
            # Ne pas l'inclure dans le mapping id_scan (oui/non)
            # 'rsn2' est la vraie question oui/non sur le scan réussi
            'id_scan' : ['rsn2', '${agent_name}, Avez pas pu scanner un nombre codes QR corresondant aux MILDA reçu dans le ménage?'],
    ...
    # CORRECTION BUG: Éviter les colonnes dupliquées après split - garder la première occurrence
    new_cols = [c.split('/')[-1] for c in data.columns]
    seen = {}
    deduped_cols = []
    for i, col in enumerate(new_cols):
        if col not in seen:
            ...
            # Suffixe unique pour les doublons
            deduped_cols.append(f"{col}__dup{i}")
    data.columns = deduped_cols
    ...
    # Traitement GPS robuste pour KoBo - supporte listes, strings et None
    if 'latitude' in data.columns:
        # CORRECTION BUG: vérification robuste, gère NaN/None en première ligne
        ...
        if first_valid is not None and isinstance(first_valid, list):
            # Format liste [lat, lon, alt, acc] depuis _geolocation KoBo
            ...
            data['longitude'] = data['longitude'].apply(
                ...
                if 'longitude' in data.columns else None
        elif first_valid is not None and isinstance(first_valid, str) and ' ' in str(first_valid):
            # Format string "lat lon alt acc" depuis geo_location KoBo
            def parse_gps_string(val):
                try:
                    parts = str(val).strip().split()
                    return float(parts[0]) if len(parts) > 0 else None
                except:
                    return None
            def parse_lon_string(val):
                try:
                    parts = str(val).strip().split()
                    return float(parts[1]) if len(parts) > 1 else None
                except:
                    return None
            # Si latitude contient la string GPS complète "lat lon alt acc"
            if 'longitude' not in data.columns or data['longitude'].isna().all():
                data['longitude'] = data['latitude'].apply(parse_lon_string)
            data['latitude'] = data['latitude'].apply(parse_gps_string)
    ...
    # APRÈS (corrigé)
    # CORRECTION BUG: indic_servi manquant dans process_milda_dataframe
    # (il existe dans load_and_process_data mais pas ici)
